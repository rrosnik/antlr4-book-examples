...
import InsertSerialIDListener from '../../src/tour/Demo/InsertSerialIDListener';
...
describe("Test For Demo example", () => {
    ...
    it("should insert serializedIds into Demo.java file", () => {
        const inputText = fs.readFileSync('./src/tour/Demo/Demo.java', 'utf8');
        const inputStream = new antlr4.CharStream(inputText);
        const lexer = new JavaLexer(inputStream);
        const tokenStream = new CommonTokenStream(lexer);
        ...
        const tree = parser.compilationUnit();
        const walker = new ParseTreeWalker();
        const listener = new InsertSerialIDListener(tokenStream);

        walker.walk(listener, tree);
        console.log(listener.rewriter.getText());
